# Quiz Re-scoring on Host Correct-Answer Change
...
Add a single field to `QuizState` (in `daemon/quiz/state.py`):

```python
self.awarded_points: dict[str, int] = {}   # pid → points awarded by the most recent reveal
```

Reset it in `__init__`, `clear()`, and `create_quiz()` alongside the other per-quiz fields.
...
1. `PersistedQuizState` (in `daemon/persisted_models.py`) gains:
...
2. In `daemon/__main__.py` snapshot writer, add:
   ...
   alongside the other quiz fields.

**Note on restore:** Quiz state is not currently rehydrated from snapshot on daemon restart (`_apply_runtime_snapshot_restore` calls `sync_from_restore` for participant/wordcloud/qa/misc/codereview/debate but **not** quiz). This feature does not change that; we simply match the existing snapshot-write pattern so `awarded_points` is in the JSON if and when quiz restore is wired up.

### Broadcast

No new WS messages. The existing `QuizCorrectRevealedMsg` (correct indices) and `ScoresUpdatedMsg` (full snapshot) already cover both effects: participants see the new correct answer and their updated score.
...
### BDD scenario (`tests/docker/features/quiz.feature`)
...
### Daemon unit tests (`tests/daemon/test_quiz_state.py`)

Three new tests, all exercising `reveal_correct` called twice on the same quiz:
...
A fourth test asserts `awarded_points` is reset by `create_quiz` and `clear()`.
...
- Host quiz tab — already re-fetches state on `QuizCorrectRevealedMsg`.
- The route `PUT /api/{session_id}/host/quiz/correct` and its `RevealCorrectRequest` body are unchanged.

## Files Touched

| File | Change |
|---|---|
| `daemon/quiz/state.py` | Add `awarded_points`; reverse-then-apply in `reveal_correct`; drop `already_revealed`. |
| `daemon/persisted_models.py` | Add `awarded_points` field to `PersistedQuizState`. |
| `daemon/__main__.py` | Add `awarded_points` to the quiz dict in `_runtime_snapshot()` (≈ line 216-224). |
| `tests/daemon/test_quiz_state.py` | 4 new unit tests. |
| `tests/docker/features/quiz.feature` | 1 new BDD scenario (`@seq`). |
| `tests/docker/features/steps/quiz_steps.py` (if needed) | Reuse existing step defs — no new glue expected. |
