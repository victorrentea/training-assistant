...
The host flow is handled transparently: `pollLocalDaemon()` detects `localhost:8081`, fetches session_id from daemon, sets the query param, and the normal flow handles the rest.
...
- **Host retry:** poll every 3s indefinitely (detected via `pollLocalDaemon` trying `localhost:8081`)
- Both error screens show a "Retrying in Xs..." countdown

**ON_HOST_MACHINE (before code entry):**
- `pollLocalDaemon()` polls `GET localhost:8081/api/session/active` every 1s
...
- Host auto-join via localhost:8081
"""
