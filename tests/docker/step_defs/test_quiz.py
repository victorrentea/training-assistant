"""
Step definitions for quiz.feature scenarios.
...
def _option_id(option_text: str) -> str:
    """Map option text to its single-letter ID (A, B, C, ...) by index in quiz."""
    options = _ctx.get("options", [])
    for i, t in enumerate(options):
        if t == option_text:
            return chr(65 + i)
    raise AssertionError(f"Option {option_text!r} not in quiz options {options!r}")
...
def _vote(pax: ParticipantPage, *options: str) -> None:
    ...
    the quiz immediately after a vote then race the daemon and miss the vote.
    ...
    pax._page.evaluate(f"""async () => {{
        const r = await fetch('/' + _sessionId + '/api/participant/quiz/vote', {{
            method: 'POST',
...
def _setup_quiz(browser, session_id, question: str, options_str: str, multi: bool) -> None:
    ...
    expect(hpage.locator("#tab-quiz")).to_be_visible(timeout=10000)
    host = HostPage(hpage)
    options = [o.strip() for o in options_str.split(";")]
    host.create_quiz(question, options, multi=multi)
    _ctx["host"] = host
...
@given(parsers.parse('a quiz "{question}" with options "{options}"'))
def given_quiz(browser, session_id, question, options):
    _setup_quiz(browser, session_id, question, options, multi=False)


@given(parsers.parse('a multi-select quiz "{question}" with options "{options}"'))
def given_multi_select_quiz(browser, session_id, question, options):
    _setup_quiz(browser, session_id, question, options, multi=True)


@given("the quiz is already open")
def given_quiz_already_open():
    expect(_host()._page.locator("#quiz-display.voting-active")).to_be_visible(timeout=5000)
...
@given("the host closes the quiz")
@when("the host closes the quiz")
def host_closes_quiz():
    _host().close_quiz()


@when(parsers.parse("the host closes the quiz with a {seconds:d} second timer via the slider"))
def host_closes_via_slider(seconds):
    """Drive the host's timer slider; wait for the host UI to reflect closure.

    Watching `#quiz-display.voting-active` (the class `renderQuizDisplay`
    toggles when the quiz opens/closes) keeps the test purely UI-driven and
...
@when(parsers.parse('the host fires a second quiz "{question}" with options "{options}"'))
def host_fires_second_quiz(browser, session_id, question, options):
    """Create another quiz while a previous one was just closed.

    Reuses the same host browser session so the bug under test (host UI state
    bleeding from one quiz into the next) can manifest. Note: we deliberately
    do NOT wait for `#quiz-display.voting-active` — the bug instantly auto-ends
    the new quiz in the host UI, so that selector flaps. We sleep briefly to
    ...
    _host()._page.evaluate(f"""async () => {{
        ...
        if (!resp.ok) throw new Error('Second quiz create failed: ' + resp.status);
    }}""")
    _ctx["options"] = new_options
    # Allow up to ~600ms for any auto-end races (host countdown ticks at 200ms)
    # to fire — without this, the test could vote before the bug closes the quiz.
    time.sleep(0.6)
...
def pax_sees_quiz():
    pax = _the_pax()
    expect(pax._page.locator(".quiz-card h2")).to_be_visible(timeout=5000)
...
def _wait_for_pax_vote_in_daemon(pax: ParticipantPage, timeout_s: float = 5) -> None:
    """Poll the daemon's host quiz state until the participant's vote is registered."""
    import json as _json
...
def _wait_for_score(pax: ParticipantPage, expected: int, timeout_ms: int = 8000) -> None:
    ...
    try:
        host_state = _host()._page.evaluate("""async () => {
            const quizR = await fetch(API('/quiz'));
            const stateR = await fetch(API('/state'));
            return {
                quiz: quizR.ok ? await quizR.json() : null,
                state: stateR.ok ? await stateR.json() : null,
            };
        }""")
        quiz = host_state.get("quiz") or {}
        ...
        debug = (
            ...
            f"  daemon_correct_indices={quiz.get('correct_indices')}\n"
