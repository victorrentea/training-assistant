"""
Hermetic E2E test: Participant quiz countdown.

Verifies:
1. Countdown appears on participant quiz card when host starts timer
2. Countdown disappears when quiz ends
"""
...
@pytest.mark.nightly
def test_quiz_countdown_visible_and_clears():
    """Participant sees countdown when host starts timer; it clears when quiz ends."""
    session_id = fresh_session("QuizCountdown")
    ...
    with sync_playwright() as p:
        ...
        host.create_quiz("Quick question?", ["Yes", "No"])
        expect(pax_raw.locator("#activity-quiz-section")).to_be_visible(timeout=5000)
        ...
        # Countdown clears after quiz ends
