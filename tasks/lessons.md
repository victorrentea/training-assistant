...
- 2026-04-11: When one endpoint currently mixes lookup and creation semantics, split them into explicit APIs (`/rejoin` for lookup-only, `/register` for create) instead of adding ambiguous mode flags.
- 2026-07-02: Restarting the macOS add-on mid-live-session breaks audio capture — a quit→relaunch races the PortAudio device handoff (`Invalid stream pointer [PaErrorCode -9988]`), leaving RMS=0.00000 and no transcription; two mid-session restarts failed to recover. A clean restart AFTER the session (Zoom/devices settled, with an ~8s gap between quit and relaunch) recovers cleanly. Lesson: never rebuild/restart the add-on while `/tmp/victor-macos-addons.log` shows live `🎙️ …words:` transcription — check that log first; defer add-on restarts to a real break. Even on an explicit "go", surface the concrete consequence (transcription will drop) before restarting into a confirmed-live session. Daemon push is far less disruptive (no participant reload unless `static/` changed).
