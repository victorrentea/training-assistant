...
- **Platform**: [Railway](https://railway.app) — redeploys **only** when `railway/**` (its watched paths) change. By design, Railway **rarely deploys**: pushes touching only `static/`, `daemon/`, `docs/`, etc. show the commit status *"No deployment needed - watched paths not modified"* and a stale `git_sha`/`railway_started_at` in `/api/status`. This is **normal**, not a broken CI/deploy.
- **How most changes reach prod (hot-deploy, no Railway redeploy)**: the daemon **auto-restarts on every push to `master`** (git pull + restart). On that restart it hot-deploys — `daemon/static_sync.py::sync_static_files` uploads changed `static/` files to the Railway proxy and broadcasts a `reload` to participant browsers; daemon-side logic goes live in-process at the same time. So `static/` (e.g. `participant.html`) and `daemon/` changes ship **automatically on push**, via the daemon, **not** via Railway — no manual restart needed.
- **New participant/host WS messages need no `railway/**` change**: they ride the generic `broadcast` envelope (`{"type":"broadcast","event":{…}}`) that Railway relays without knowing the event type. Only change `railway/**` (and thus trigger a real Railway deploy) when the relay/auth/routing itself must change.
...
- **Votes are mutable**: participants can change their vote at any time while the quiz is open; the last vote timestamp is used for scoring
