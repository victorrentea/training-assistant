...
- **Versioning**: a GitHub Action on push to master generates both `static/version.js` (timestamp) and `static/deploy-info.json` (changelog), committed by the bot with `[skip ci]`; both host and participant pages display the version age in the bottom-right corner
