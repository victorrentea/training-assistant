...
| Message | Payload |
| --- | --- |
| Host-only poll snapshot — always carries full counts regardless of public flag<br>Sent exclusively to the host channel (notify_host). Participants never receive this.<br>`counts[i]` is the number of votes for `poll.options[i]` (participant_counts[i] + host_extras[i]).<br>`poll` is null when the host cleared the draft; `started` reflects whether a poll is currently live.<br>`poll_host_update` | `poll?: Poll{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`question:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`options:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`multi:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`public:bool  # When true, counts are broadcast to participants`<br>`}`<br>`started?: bool  # True while a poll is live; false after Stop or Clear`<br>`counts?: list[int]  # Per-option total vote counts (participant + host extras)`<br>`voted_count?: int  # Total number of distinct participants who have voted`<br>`participant_counts?: list[int]  # Per-option vote counts from real participants only`<br>`host_extras?: list[int]  # Per-option host-added extras (e.g. for chat answers); total = participant + extras` |
