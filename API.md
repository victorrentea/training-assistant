...
| Register Participant, idempotent for returning participants.<br>`POST /api/participant/register` | `name?: string`<br>`location?: string` | `name: string`<br>`avatar: string`<br>`name_conflict?: bool` |
| Rejoin Participant, lookup-only identity restore for returning UUIDs in current session.<br>`POST /api/participant/rejoin` | - | `name: string`<br>`avatar: string`<br>`name_conflict?: bool` |
