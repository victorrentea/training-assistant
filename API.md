...
| Host requests a quiz — stores request for the orchestrator loop to pick up.<br>`POST /api/{session_id}/host/quiz-request` | `minutes?: int`<br>`topic?: string` | - |
...
| Get Host Summary, return summary points, raw markdown, and updated_at timestamp.<br>`GET /api/{session_id}/host/summary` | - | `points?: list[SummaryPoint]`<br>`raw_markdown?: string`<br>`updated_at?: string`<br><br>`SummaryPoint {`<br>`  text: string`<br>`  source: string`<br>`}` |
