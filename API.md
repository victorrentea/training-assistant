...
| Host starts a new session (creates folder, assigns session_id, clean<br>slate).<br>`POST /api/session/create` | `name: string`<br>`type: 'workshop' \| 'talk'` | `session_name: string`<br>`session_id: string` |
...
| Talk Presentation Path, host drops a PPTX file during a talk<br>— resolve GDrive URL, trigger Railway download, push slides_current.<br>`POST /api/session/talk-presentation-path` | `path: string` | - |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Resolve Participant Locations, backfill city name + timezone + country<br>for all participants missing geo metadata.<br>`POST /api/{session_id}/host/participants/resolve-locations` | - | - |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Slides Compilation, compile all viewed slide pages into one PDF and<br>return as a download;<br>long-running: may trigger Railway to download PDFs from Google Drive<br>first;<br>progress is logged to the daemon log.<br>`GET /api/{session_id}/host/slides-compilation` | - | `any` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Clear Queue<br>`DELETE /api/{session_id}/host/quiz-queue` | - | `ok?: bool` |
| Get Queue Status, return how many questions are pending and what the<br>current question looks like.<br>`GET /api/{session_id}/host/quiz-queue` | - | `pending: int`<br>`current?: QuizQueueQuestion {`<br>&nbsp;&nbsp;&nbsp;&nbsp;`question:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`options:list[QuizQueueOption {`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`}]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`correct_ids:list[string]`<br>`}` |
| Submit Questions, replace the entire quiz queue with the submitted<br>questions.<br>`POST /api/{session_id}/host/quiz-queue` | `questions: list[QuizQueueQuestion {`<br>&nbsp;&nbsp;&nbsp;&nbsp;`question:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`options:list[QuizQueueOption {`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`}]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`correct_ids:list[string]`<br>`}]` | `ok?: bool` |
| Fire Current<br>`POST /api/{session_id}/host/quiz-queue/fire` | - | `ok?: bool` |
| Skip Current<br>`POST /api/{session_id}/host/quiz-queue/skip` | - | `ok?: bool` |
