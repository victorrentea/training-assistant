...
| Host starts a new session (creates folder, assigns session_id, clean slate). For fresh sessions the local folder is created immediately so DriveFS can sync it up to Google Drive; we then poll DriveFS for the resulting URL up to 15s before returning. The host UI only navigates into the session once we respond, so this guarantees a usable gdrive_url is in place.<br>`POST /api/session/create` | `name: string`<br>`type: 'workshop' \| 'talk'` | `session_name: string`<br>`session_id: string` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Report Activity, merge a participant's per-view engagement deltas (active seconds/visits/clicks); called by the participant page at most every ~30s while active, and on tab-hide/unload; idle/backgrounded tabs send nothing.<br>`POST /api/participant/activity` | `current_view?: string`<br>`deltas?: dict[str, ViewEngagementDelta{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`seconds?:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`visits?:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`clicks?:int`<br>`}]` | - |
| Set Location, store participant city/timezone.<br>`PUT /api/participant/location` | `location: string` | - |
| Rename Participant, returns 400 if not yet registered; A duplicate name is accepted (never a 409): the response carries a soft `name_conflict` flag and the participant stays admitted.<br>`PUT /api/participant/name` | `name: string` | `name_conflict?: bool` |
| Register Participant, idempotent for returning participants.<br>`POST /api/participant/register` | `name?: string`<br>`location?: string` | `name: string`<br>`name_conflict?: bool` |
| Rejoin Participant, lookup-only identity restore for returning UUIDs in current session.<br>`POST /api/participant/rejoin` | - | `name: string`<br>`name_conflict?: bool` |
...
| Roster display names changed (join / rename / leave)<br>SECURITY: display names only — never UUIDs or any stable per-user id (a leaked UUID enables impersonation via X-Participant-ID).<br>In-session duplicate detection is UUID-free: count occurrences of your own name in 'names' (>=2 => duplicate).<br>`participant_names_updated` | `names: list[string]` |
...
| Message | Payload |
| --- | --- |
| Participant list changed (join/register/rename/location) — sent by daemon directly<br>`participant_list_updated` | `participants: list[HostParticipant{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`uuid:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`name:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`score:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`location?:string`<br>`}]` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Participant State, return full personalised state for a participant — used on page load and WS reconnect; returns participant-personalized full state snapshot.<br>`GET /api/participant/state` | - | `mode: string`<br>`my_score: int`<br>`my_score_token: string`<br>`my_name: string`<br>`current_activity: string`<br>`session_name?: string`<br>`participant_names?: list[string]`<br>`wordcloud: WordcloudData{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`words:dict[str, int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`word_order:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`topic:string`<br>`}`<br>`qa_questions: list[QAQuestionParticipant{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`upvote_count:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`answered:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`timestamp:number`<br>&nbsp;&nbsp;&nbsp;&nbsp;`is_own:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`has_upvoted:bool`<br>`}]`<br>`quiz?: QuizData{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`question:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`options:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`multi:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`correct_count?:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`end_timer_seconds?:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`end_timer_started_at?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`correct_indices?:list[int]`<br>`}`<br>`quiz_active: bool`<br>`my_voted_indices?: list[int]`<br>`quiz_correct_indices?: list[int]`<br>`poll?: dict[str, any]`<br>`poll_active?: bool`<br>`my_poll_voted_indices?: list[int]`<br>`poll_vote_counts?: list[int]`<br>`codereview: CodeReviewParticipantState{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`snippet?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`language?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`phase?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`confirmed_lines?:list[int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`my_selections?:list[int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`line_percentages?:dict[str, int]`<br>`}`<br>`debate: DebateData{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`statement?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`phase?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`my_side?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`my_is_champion:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`my_auto_assigned?:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`side_counts:dict[str, int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`arguments:list[DebateArgumentParticipant{`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`side:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`upvote_count:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`ai_generated:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`merged_into?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`is_own:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`has_upvoted:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`}]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`champions:dict[str, bool]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`first_side?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`round_index?:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`round_timer_seconds?:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`round_timer_started_at?:string`<br>`}`<br>`slides_current?: CurrentSlide{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`slug:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`page:int`<br>`}`<br>`talk_presentation_slug?: string`<br>`notes_updated_at?: string`<br>`summary_updated_at?: string`<br>`wiki_updated_at?: string`<br>`summary_scroll?: SummaryScrollPosition{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`block_key:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`path?:list[int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`within?:number`<br>&nbsp;&nbsp;&nbsp;&nbsp;`summary_updated_at?:string`<br>`}`<br>`slides_history_count: int`<br>`files_count?: int`<br>`prompts_count?: int`<br>`gdrive_url?: string`<br>`feedback_url?: string`<br>`has_agenda?: bool`<br>`emoji_catalog: list[EmojiDef{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`emoji:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`title:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`section:'primary' \| 'signal' \| 'overflow'`<br>&nbsp;&nbsp;&nbsp;&nbsp;`badge?:string`<br>`}]`<br>`attention_enabled?: bool` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Host State, return full state for host page load — replicates Railway build_for_host(); returns host-facing full state snapshot.<br>`GET /api/{session_id}/host/state` | - | `type?: string`<br>`mode: string`<br>`current_activity: string`<br>`participant_count: int`<br>`participants: list[HostParticipant{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`uuid:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`name:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`score:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`location:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`location_tz?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`location_country?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`paste_texts?:list[PasteEntry{`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`}]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`received_files?:list[UploadedFileEntry{`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`filename:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`size:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`disk_path:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`seen_by_host:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`}]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`engagement?:dict[str, dict[str, any]]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`last_active_at?:number`<br>&nbsp;&nbsp;&nbsp;&nbsp;`last_view?:string`<br>`}]`<br>`emoji_global_enabled?: bool`<br>`attention_enabled?: bool`<br>`daemon_connected: bool`<br>`railway_connected: bool`<br>`overlay_connected: bool`<br>`gdrive_running: bool`<br>`wordcloud_words: dict[str, int]`<br>`wordcloud_word_order: list[string]`<br>`wordcloud_topic: string`<br>`qa_questions: list[HostQAQuestion{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`author:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`author_uuid:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`upvote_count:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`upvoters:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`answered:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`timestamp:number`<br>`}]`<br>`codereview: HostCodeReviewState{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`snippet?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`language?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`phase?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`confirmed_lines?:list[int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`selections?:dict[str, list[int]]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`line_percentages?:dict[str, int]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`line_counts?:dict[str, int]`<br>`}`<br>`debate_statement?: string`<br>`debate_phase?: string`<br>`debate_side_counts: dict[str, int]`<br>`debate_sides: dict[str, string]`<br>`debate_arguments: list[DebateArgumentHost{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`author_uuid:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`side:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`upvoters:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ai_generated:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`merged_into?:string`<br>`}]`<br>`debate_champions: dict[str, string]`<br>`debate_auto_assigned: list[string]`<br>`debate_first_side?: string`<br>`debate_round_index?: int`<br>`debate_round_timer_seconds?: int`<br>`debate_round_timer_started_at?: string`<br>`talk_presentation_name?: string`<br>`talk_presentation_slug?: string`<br>`slides_log_deep_count: int`<br>`slides_log_topic?: string`<br>`session_id?: string`<br>`join_base_url: string`<br>`daemon_session_folder?: string`<br>`daemon_session_notes?: string`<br>`session_type?: string`<br>`notes_updated_at?: string`<br>`summary_updated_at?: string`<br>`transcript_line_count: int`<br>`transcript_total_lines: int`<br>`transcript_latest_ts?: string` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Refresh slide PDF cache from Google Drive, unified entry point for both the participant cache-miss path and the daemon's PPTX-watcher path; marks the slug stale, deletes the old cached PDF, downloads from Drive, and returns the new SHA256; per-slug deduplication: parallel calls for the same slug share one in-flight fetch; the daemon receives a slide_log "completed" event over WS and broadcasts decks_updated to participants.<br>`POST /api/slides/refresh/{slug}` | `drive_export_url: string  # Google Drive PDF export URL for the slide deck.` | `status: string  # Always "cached" on success.`<br>`sha256: string  # SHA-256 hex digest of the cached PDF file.`<br>`size: int  # File size in bytes.` |
...
| Poll snapshot — fires on start, host edits, stop, and votes when public=true<br>`counts` is present when `poll.public` is true OR when the poll has been ended (read-only results until Clear); absent → hide per-option bars.<br>`counts[i]` is the number of votes for `poll.options[i]`.<br>`ended=true` flips the participant UI to a read-only results view that lingers until host clicks Clear.<br>`poll_updated` | `poll: Poll{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`question:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`options:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`multi:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`public:bool  # When true, counts are broadcast to participants`<br>`}`<br>`counts?: list[int]  # Per-option vote counts; present when poll.public OR poll has been ended`<br>`ended?: bool  # True after host clicked Stop, until Clear is clicked. Participant view becomes read-only.` |
...
| Stop Poll, end the live poll but keep the draft, votes and host_extras so participants and host both see the final results read-only until Clear; current_activity stays "poll" so participants do not get routed away from the view.<br>`POST /api/{session_id}/host/poll/stop` | - | - |
...
| Message | Payload |
| --- | --- |
| Host-only poll snapshot — always carries full counts regardless of public flag<br>Sent exclusively to the host channel (notify_host). Participants never receive this.<br>`counts[i]` is the number of votes for `poll.options[i]` (participant_counts[i] + host_extras[i]).<br>`poll` is null when the host cleared the draft; `started` reflects whether a poll is currently live.<br>`ended=true` after host clicked Stop, until Clear; counts + draft preserved, edits locked.<br>`poll_host_update` | `poll?: Poll{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`question:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`options:list[string]`<br>&nbsp;&nbsp;&nbsp;&nbsp;`multi:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`public:bool  # When true, counts are broadcast to participants`<br>`}`<br>`started?: bool  # True while a poll is live; false after Stop or Clear`<br>`ended?: bool  # True after host clicked Stop, until Clear. Host UI locks edits and keeps showing results.`<br>`counts?: list[int]  # Per-option total vote counts (participant + host extras)`<br>`voted_count?: int  # Total number of distinct participants who have voted`<br>`participant_counts?: list[int]  # Per-option vote counts from real participants only`<br>`host_extras?: list[int]  # Per-option host-added extras (e.g. for chat answers); total = participant + extras` |
...
| Message | Payload |
| --- | --- |
| Q&A list changed (new question, upvote, edit, delete)<br>`qa_updated` | `questions: list[QAQuestion{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`upvote_count:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`answered:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`timestamp:number`<br>`}]` |
...
| Message | Payload |
| --- | --- |
| UUID-free debate state snapshot (participant-facing)<br>`debate_updated` | `statement?: string`<br>`phase?: 'side_selection' \| 'arguments' \| 'ai_cleanup' \| 'prep' \| 'live_debate' \| 'ended'`<br>`side_counts?: dict[str, int]  # Aggregate members per side ("for"/"against" → count). Replaces the old uuid→side map — this frame carries NO participant UUIDs.`<br>`arguments?: list[DebateArgument{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`id:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`side:'for' \| 'against'`<br>&nbsp;&nbsp;&nbsp;&nbsp;`text:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`upvote_count:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`ai_generated:bool`<br>&nbsp;&nbsp;&nbsp;&nbsp;`merged_into?:string`<br>`}]`<br>`champions?: dict[str, bool]  # "for"\|"against" → whether that side has claimed a champion (no uuid)`<br>`first_side?: string`<br>`round_index?: int`<br>`round_timer_seconds?: int`<br>`round_timer_started_at?: string` |
...
| Message | Payload |
| --- | --- |
| One or more participants' scores changed<br>`scores_updated` | `scores: dict[str, int]  # opaque per-session score token → score. NOT keyed by UUID — each participant learns only its own token via GET /state (my_score_token) and reads scores[my_score_token]. The token cannot be replayed as an X-Participant-ID identity.` |
| Leaderboard overlay shown with top positions<br>`leaderboard_revealed` | `positions: list[LeaderboardPosition{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`rank:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`name:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`score:int`<br>`}]` |

### Host REST
| Endpoint | Request | Response |
| --- | --- | --- |
| Show Leaderboard<br>`POST /api/{session_id}/host/leaderboard/show` | - | `entries: list[LeaderboardPosition{`<br>&nbsp;&nbsp;&nbsp;&nbsp;`rank:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`name:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`score:int`<br>&nbsp;&nbsp;&nbsp;&nbsp;`universe?:string`<br>&nbsp;&nbsp;&nbsp;&nbsp;`is_trainer?:bool`<br>`}]` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Toggle Emoji Global, host flips the session-wide emoji master switch and persists it.<br>`POST /api/{session_id}/host/emoji/global-toggle` | - | `emoji_global_enabled: bool` |
...
| A fresh addon-stamped snippet was appended to the session notes — toast it for copy (hand-typed notes lines are not broadcast)<br>`notes_appended` | `text: string  # The newly-appended snippet (bullet stripped, trimmed, addon source marker 🤖/📋 kept)`<br>`at: string  # ISO timestamp when the daemon observed the append` |
...
| Host scrolled the summary — followers scroll to the same bullet<br>`summary_scroll` | `block_key: string  # data-key of the top-level summary block at the top of the host's view`<br>`path?: list[int]  # Bullet indices from that block down to the anchor bullet`<br>`within?: number  # Fraction of the anchor already scrolled past the top of the host's view`<br>`summary_updated_at?: string  # Summary version the host had rendered` |
...
| The session wiki was published from the session's wiki/ folder, or withdrawn<br>`wiki_updated` | `updated_at?: string  # ISO timestamp of the last successful publish; null hides the Wiki menu entry` |
...
| Summary Scroll Local<br>`POST /summary/scroll` | `block_key: string`<br>`path?: list[int]`<br>`within?: number`<br>`summary_updated_at?: string` | `block_key: string`<br>`path?: list[int]`<br>`within?: number`<br>`summary_updated_at?: string` |
...
| Get Prompts, return the agent prompts intercepted this session, oldest first; they are read back out of the same session notes file the macOS addon appends them to (each stamped 🤖), so the room sees exactly what was sent to the agent — no second store to keep in sync with the notes.<br>`GET /api/participant/prompts` | - | `prompts: list[string]`<br>`updated_at?: string` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Participant rings the attention bell (no request body).<br>`POST /api/participant/bell` | - | - |

### Participant WS
| Message | Payload |
| --- | --- |
| Host flipped the attention master switch — participants update live<br>Carries a boolean only (no UUIDs). Show/hide the bell button and the notification-permission indicator without reloading the page.<br>`attention_enabled` | `enabled: bool  # Whether the attention capability is now on` |
| Host broadcast a text notification to all participants<br>Carries free text + a timestamp only (no UUIDs). Rendered as a native OS Notification when permission is granted, otherwise an in-page toast; both paths play a sound.<br>`host_notification` | `text: string  # The host's notification message`<br>`at: string  # ISO-8601 timestamp when the daemon broadcast it` |

### Host REST
| Endpoint | Request | Response |
| --- | --- | --- |
| Toggle Attention Global, host flips the session-wide attention master switch, persists it, and broadcasts the new state so every connected participant updates live.<br>`POST /api/{session_id}/host/attention/global-toggle` | - | `attention_enabled: bool` |
| Send Host Notification, host broadcasts a text notification to all participants (Direction A).<br>`POST /api/{session_id}/host/attention/notify` | `text: string` | - |
...
| Message | Payload |
| --- | --- |
| Participant rang the attention bell — overlay alerts the host<br>Fire-and-forget, best-effort. If the overlay is not connected the daemon logs the drop and still returns success to the participant.<br>On receipt the overlay plays a bell sound and shows a persistent, hover-dismissible bottom-left card reading "🔔 [caller] is calling you".<br>`bell_ring` | `caller: string  # Resolved display name of the ringing participant (never the raw UUID when a name is known)` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Fx Fire, press the selected tile, if the presser holds the button and all the brakes are off.<br>`POST /api/participant/fx/fire` | - | `fired: bool`<br>`reason: string`<br>`ready_in_seconds: int` |
| Fx Info, what one participant's page renders: whether they hold the button, and what shape it is in.<br>`GET /api/participant/fx/info` | - | `granted: bool`<br>`tile_n: int`<br>`label: string`<br>`effect?: string`<br>`enabled: bool`<br>`cooldown_seconds: int`<br>`ready_in_seconds: int`<br>`effects_up: bool`<br>`tile_available: bool` |

### Participant WS
| Message | Payload |
| --- | --- |
| Something about the FX button moved — re-read your own /info<br>Deliberately empty. Broadcast to the whole room, so it cannot name who was granted or revoked without breaking the no-UUID-to-participants invariant; each page asks about itself via `GET /api/participant/fx/info` with its own X-Participant-ID.<br>Sent on grant, revoke, grant-all, revoke-all, master toggle, tile change and cooldown change.<br>`fx_changed` | - |
| The shared FX lever just fired — every granted button greys out<br>There is one cooldown for the whole room, not one per grantee, so a press by anyone has to grey out everyone else's button — otherwise the other grantees discover the cooldown only by pressing into it.<br>Says how long, never who. No UUIDs.<br>`fx_cooling` | `ready_in_seconds: int  # Seconds until the button works again, for anyone` |
...
| Fx Set Cooldown, change how often the room may pull the lever.<br>`POST /api/{session_id}/host/fx/cooldown` | `seconds: int` | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx Grant, hand the button to one participant.<br>`POST /api/{session_id}/host/fx/grant` | `participant_id: string` | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx Grant All, hand the button to the whole roster; the roster, not just whoever is online this second: someone who steps out and comes back would otherwise find their button gone for reasons nobody could explain; the cooldown is what keeps this survivable — see `cooldown_remaining`.<br>`POST /api/{session_id}/host/fx/grant-all` | - | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx Revoke, take it back from one participant; their press count survives: it is the session's record of what happened, not a property of the grant, and re-granting someone must not silently reset how many times they have already leaned on it.<br>`POST /api/{session_id}/host/fx/revoke` | `participant_id: string` | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx Revoke All, take the button away from everyone — the panic button for the panic button.<br>`POST /api/{session_id}/host/fx/revoke-all` | - | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx State, everything the footer badge, its popover and the roster's toggles render.<br>`GET /api/{session_id}/host/fx/state` | - | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx Test, fire the selected tile from the host page; deliberately ignores every brake: you check the wiring precisely when the switch is off and nobody is granted, and a cooldown meant for the room should not make the trainer wait; it does not start a cooldown either — testing must not take the lever away from the room; it is also not counted against anybody: the counters answer "who is leaning on this", and the trainer testing his own soundboard is not an answer to that; this is also the room's only recovery path once a real press has failed: /info trusts the last actual attempt over the ping (see _effects_reachable()), so after a failure the participants' buttons stay disabled until a Test from here proves the wiring works again.<br>`POST /api/{session_id}/host/fx/test` | - | `fired: bool`<br>`reason: string`<br>`ready_in_seconds: int` |
| Fx Set Tile, bind the button to a different tile.<br>`POST /api/{session_id}/host/fx/tile` | `n: int` | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |
| Fx Toggle, arm or disarm every grant at once, without touching who holds one.<br>`POST /api/{session_id}/host/fx/toggle` | - | `enabled: bool`<br>`tile_n: int`<br>`tile_label: string`<br>`effect?: string`<br>`cooldown_seconds: int`<br>`last_fired_at?: number`<br>`effects_up: bool`<br>`granted: list[string]`<br>`press_counts: dict[str, int]` |

### Host WS
| Message | Payload |
| --- | --- |
| A granted participant pressed the FX button<br>`caller` is a resolved display name, never the raw UUID rendered as a name; an unknown or unnamed presser is "Someone".<br>`uuid` rides alongside it because this is a HOST frame, where UUIDs are expected — the roster needs a key to bump the right row's counter.<br>`fx_fired` | `tile_n: int`<br>`label: string`<br>`at: number  # Unix timestamp (time.time()) when the tile was pressed`<br>`caller?: string  # Resolved display name of whoever pressed the button ("Someone" when unknown)`<br>`anonymous?: bool  # Whether that participant joined anonymously`<br>`uuid?: string  # The presser's participant UUID (host channel only — keys the roster row)`<br>`count?: int  # How many times that participant has pressed it this session` |

### Addons WS
| Message | Payload |
| --- | --- |
| Someone pulled the secret FX link — overlay names who pressed it<br>Fire-and-forget, best-effort. A closed overlay never fails the press — the participant is still told it fired.<br>Sent only after the press itself succeeded, on the *other* edge to the Mac (HTTP `GET :55123/press/<n>`). A refused press (disabled / cooling / unknown tile / effects down) sends nothing.<br>`caller` is a display name resolved by the daemon from the UUID the FX page reads out of its own localStorage. Never a raw UUID: an unknown, unnamed or absent holder is "Someone".<br>On receipt the overlay shows a red bottom-center tab reading "[caller]" — the name alone, no glyph and no tile — that rises, holds 3s and falls away. No chime, and no tile name: the tile's own sound is already playing, so the tab carries only what the room cannot supply.<br>`fx_fired` | `caller: string  # Resolved display name of whoever pressed the button ("Someone" when unknown)`<br>`anonymous?: bool  # Whether that participant joined anonymously` |
...
| Message | Payload |
| --- | --- |
| Number of agent prompts intercepted this session changed<br>Sourced from the 🤖-stamped lines of the session notes file; moves when the trainer confirms a captured prompt.<br>`prompts_count_updated` | `count: int` |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Publish the session wiki (a static Quartz site built by the daemon), sent by the daemon whenever the session folder's wiki/ vault has changed and settled (and again, with the cached build, after a Railway reconnect); replaces the session's site, deletes every other session's, and serves it to participants of the ACTIVE session only at GET /{session_id}/wiki-site/{path}; zip-slip paths are refused; 40 MB zip / 150 MB unpacked ceiling.<br>`POST /api/wiki/upload` | `session_id: string  # Active session the site belongs to (alphanumeric).`<br>`file: string  # Zipped Quartz output folder.` | `ok: bool`<br>`files?: int  # Number of files unpacked.` |
