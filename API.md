...
| Get Session Active<br>`GET /api/session/active` | - | `any`<br>Note: Public endpoint: returns the active session_id or null. |
| End Session<br>`POST /api/session/end` | - | `any`<br>Note: Host ends the current session. Railway closes WS connections on session end. |
| End Talk<br>`POST /api/session/end_talk` | - | `any`<br>Note: Host ends the nested talk. |
| List Session Folders<br>`GET /api/session/folders` | - | `any`<br>Note: List available session folders. |
...
| Start Talk<br>`POST /api/session/start_talk` | - | `any`<br>Note: Host starts a nested talk (conference mode). |
| Set Mode<br>`POST /api/{session_id}/host/mode` | `{mode: string}` | `any`<br>Note: Host switches session mode (workshop/conference). |
| Get Interval Lines Txt<br>`GET /api/{session_id}/session/interval-lines.txt` | - | `text/plain: string`<br>Note: Return raw transcript lines for a time window.<br>Note: Returns text/plain interval lines for session export/inspection. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Slides Cache Status<br>`GET /api/participant/slides-cache-status` | - | `any`<br>Note: Get slides cache status.<br>Note: Primarily for diagnostics; UI cache invalidation is event-driven via slides_cache_status WS. |
...
| Register Participant<br>`POST /api/participant/register` | - | `{name: string, avatar: string}`<br>Note: Register participant — assign name+avatar. Idempotent for returning participants. |
| Get Participant State<br>`GET /api/participant/state` | - | `any`<br>Note: Return full personalised state for a participant — used on page load and WS reconnect.<br>Note: Returns participant-personalized full state snapshot. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Host State<br>`GET /api/{session_id}/host/state` | - | `any`<br>Note: Return full state for host page load — replicates Railway build_for_host().<br>Note: Returns host-facing full state snapshot. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Delete Poll<br>`DELETE /api/{session_id}/host/poll` | - | `any`<br>Note: Host deletes the current poll. |
...
| Close Poll<br>`POST /api/{session_id}/host/poll/close` | - | `any`<br>Note: Host closes the poll. |
...
| Open Poll<br>`POST /api/{session_id}/host/poll/open` | - | `any`<br>Note: Host opens the poll for voting. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Clear Wordcloud<br>`POST /api/{session_id}/host/wordcloud/clear` | - | `any`<br>Note: Host clears the word cloud. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Clear Qa<br>`POST /api/{session_id}/host/qa/clear` | - | `any`<br>Note: Host clears all Q&A questions. |
| Delete Question<br>`DELETE /api/{session_id}/host/qa/question/{question_id}` | - | `any`<br>Note: Host deletes a question. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Clear Codereview<br>`DELETE /api/{session_id}/host/codereview` | - | `any`<br>Note: Host clears the code review. |
...
| Volunteer Champion<br>`POST /api/participant/debate/volunteer` | - | `any`<br>Note: Participant volunteers as champion for their side. |
...
| Close Selection<br>`POST /api/{session_id}/host/debate/close-selection` | - | `any`<br>Note: Host closes side selection; auto-assigns remaining participants. |
| End Arguments<br>`POST /api/{session_id}/host/debate/end-arguments` | - | `any`<br>Note: Host ends arguments phase; triggers AI cleanup in background. |
| End Round<br>`POST /api/{session_id}/host/debate/end-round` | - | `any`<br>Note: Host ends the current round early. |
...
| Force Assign<br>`POST /api/{session_id}/host/debate/force-assign` | - | `any`<br>Note: Host force-assigns all unassigned participants. |
...
| Reset Debate<br>`POST /api/{session_id}/host/debate/reset` | - | `any`<br>Note: Host resets all debate state. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Clear Quiz Preview<br>`DELETE /api/{session_id}/host/quiz-preview` | - | `any`<br>Note: Host clears the current quiz preview. |
...
| Get Quiz Md<br>`GET /api/{session_id}/quiz-md` | - | `any`<br>Note: Return the accumulated quiz markdown history. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Pastes<br>`GET /api/{session_id}/host/pastes` | - | `any`<br>Note: Return all pending paste entries grouped by participant uuid. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Notes<br>`GET /api/participant/notes` | - | `any`<br>Note: Get session notes content. |
| Get Summary<br>`GET /api/participant/summary` | - | `any`<br>Note: Get summary points and raw markdown. |
...
| Endpoint | Request | Response |
| --- | --- | --- |
| Get Host Notes<br>`GET /api/{session_id}/host/notes` | - | `any`<br>Note: Return current session notes content. |
| Get Host Summary<br>`GET /api/{session_id}/host/summary` | - | `any`<br>Note: Return summary points, raw markdown, and updated_at timestamp. |
...
| Poll Transcription Language Request<br>`GET /api/transcription-language/request` | - | `any`<br>Note: Daemon/macos-addons polls for a pending language change request (clears on read).<br>Note: Consumes and clears the pending transcription language request. |
