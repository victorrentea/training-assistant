...
function closeEmojiPopup(ev) {
  ...
  function runOnboardingTourIfNeeded() {
    ...
    function positionBubble(bub, anchor, step = null) {
      ...
      if (step?.arrowRight) {
        ...
        const bubbleLeft = Math.max(8, rect.left - bubW - 14);
        bub.style.left = bubbleLeft + 'px';
        bub.style.top = Math.max(8, Math.min(window.innerHeight - 120, rect.top + rect.height / 2 - 30)) + 'px';
        return;
      }
  ...
  function _wasSlidesOverlayOpen() {
    return localStorage.getItem(LS_SLIDES_OVERLAY_OPEN) === '1';
  }

  function _setSlidesOverlayOpen(open) {
    if (open) {
      localStorage.setItem(LS_SLIDES_OVERLAY_OPEN, '1');
    } else {
      localStorage.removeItem(LS_SLIDES_OVERLAY_OPEN);
    }
  }
  ...
  function _suppressSlidesZoom(ms = 1000) {
    slidesZoomInProgressUntil = Date.now() + Math.max(0, Number(ms || 0));
    _suppressSlidesFollowAutoUncheck(ms);
  }

  function _isSlidesZoomInProgress() {
    return Date.now() < slidesZoomInProgressUntil;
  }
  ...
  function _renderSlidesViewModeToggle() {
    ...
    if (!nativeBtn || !pdfjsBtn) return;
    const isNative = slidesViewMode === 'native';
  ...
  async function _initSlidesViewer() {
    ...
    slidesPdfEventBus.on('pagechanging', (evt) => {
      if (slidesSelectedSlug && evt?.pageNumber) {
        ...
        if (_isSlidesZoomInProgress()) return;
  ...
  function _buildSlideItem(slide, targetId) {
  ...
  function _renderSlidesListFlat(targetId) {
    ...
    for (const slide of slidesCatalog) {
      list.appendChild(_buildSlideItem(slide, targetId));
    }
    _markSelectedSlideInList();
  }
  ...
  function _renderSlidesList(targetId) {
    ...
    for (const slide of slidesCatalog) {
      if (slide.group && slide.group !== currentGroup) {
        ...
        currentGroupEl.className = 'slides-group-block' + (groupCounts[slide.group] === 1 ? ' solo' : '');
      ...
      if (slide.group) {
        currentGroupEl.appendChild(_buildSlideItem(slide, targetId));
      } else {
        list.appendChild(_buildSlideItem(slide, targetId));
        currentGroup = null;
        currentGroupEl = null;
  ...
  async function _refreshSlidesCatalog({ forceReloadCurrent = false, autoLoadSelected = false } = {}) {
    ...
    try {
      ...
      const targetId = autoLoadSelected && selectedStillExists ? rememberedId : null;
      if (targetId) slidesSelectedId = targetId;
      _renderSlidesList(targetId || null);
      if (targetId) {
  ...
  function warmSlidesCatalog() {
    const shouldRestoreOpen = _wasSlidesOverlayOpen();
    const overlay = document.getElementById('slides-overlay');
