...
C4Context
    ...
    Person(host, "Host", "Manages activities, views results, controls debate and code review.")
    Person(participant, "Participant", "Trainee.")
    System(workshop, "Workshop Live Interaction Tool", "Real-time trainee interaction platform for live workshops.")
    System_Ext(audio_hijack, "Audio Hijack", "Captures and transcribes live workshop audio to a local file on host's Mac.")
    System_Ext(claude_api, "Anthropic Claude API", "LLM for quiz generation, debate cleanup, summaries.")
    System_Ext(nominatim, "Nominatim (OpenStreetMap)", "GPS to city + country.")
    System_Ext(google_drive, "Google Drive", "Hosts public PDF exports of presentation slides.")
...
C4Container
    ...
    System_Boundary(workshop, "Workshop Tool") {
        Container(participant_spa, "Participant SPA", "Vanilla JS (trainee's browser)", "Voting, Q&A, word cloud, debate, code review, geolocation, emoji reactions.")
        Container(fastapi, "FastAPI Backend", "Python 3.12 / FastAPI / Uvicorn", "REST endpoints + WebSocket. HTTP Basic Auth via middleware. TLS handled by Railway.")
        Container(host_spa, "Host SPA", "Vanilla JS (host's browser)", "Poll/activity management, participant list, map view (Leaflet), quiz preview, debate control.")
        Container(training_daemon, "Daemon", "Python 3.12 CLI (host's machine)", "Long-polls backend for quiz requests and debate AI cleanup. Posts AI-generated content to backend. Manages slides PPTX to PDF conversion and upload.")
        Container(emoji_overlay, "Emoji Overlay", "Swift / AppKit (host's Mac)", "Transparent always-on-top window. Receives emoji reactions via WebSocket and animates them over the screen.")
    }

    System_Ext(audio_hijack, "Audio Hijack", "Captures live audio and writes the transcript file.")
    System_Ext(claude_api, "Anthropic Claude API", "LLM for quiz generation, debate argument cleanup, and session summaries.")
    System_Ext(nominatim, "Nominatim", "GPS coords to city + country.")
    System_Ext(google_drive, "Google Drive", "Hosts public PDF exports of presentation slides.")
    ...
    Rel(participant_spa, nominatim, "Reverse geocodes GPS to city+country", "HTTPS REST")
    Rel(host, training_daemon, "Starts/stops", "Local terminal")
    Rel(audio_hijack, training_daemon, "Writes transcript file to disk (daemon reads local filesystem)", "Local file")
    Rel(training_daemon, fastapi, "Polls for requests, posts preview and slides upload", "HTTPS REST + WSS")
    Rel(training_daemon, claude_api, "Generates quiz, debate AI cleanup, summary", "HTTPS REST")
    Rel(fastapi, google_drive, "Downloads PDF exports of presentation slides", "HTTPS")
...
C4Component
    ...
    Container_Boundary(backend, "FastAPI Backend") {
        Component(core, "core/", "Python package", "Shared infrastructure: state.py (AppState singleton), auth.py (HTTP Basic Auth), metrics.py (Prometheus), names.py (conference mode names), messaging.py (registry + broadcast), state_builder.py (core WS state)")
        Component(ws, "features/ws", "FastAPI router", "WebSocket endpoints: /ws/{uuid} (participant, host, overlay), /ws/daemon (daemon heartbeat + slide upload). Dispatches all real-time messages. Sends personalized state on connect.")
        Component(poll, "features/poll", "FastAPI router", "POST /api/poll, PUT /api/poll/status, PUT /api/poll/correct, POST /api/poll/timer, DELETE /api/poll, GET /api/suggest-name, GET /api/status")
        Component(qa, "features/qa", "FastAPI router", "PUT /api/qa/question/{id}/text, DELETE /api/qa/question/{id}, PUT /api/qa/question/{id}/answered, POST /api/qa/clear")
        Component(wordcloud, "features/wordcloud", "FastAPI router", "POST /api/wordcloud/topic, POST /api/wordcloud/clear")
        Component(codereview, "features/codereview", "FastAPI router", "POST /api/codereview (smart paste), PUT /api/codereview/status, PUT /api/codereview/confirm-line, DELETE /api/codereview")
        Component(debate, "features/debate", "FastAPI router", "POST /api/debate, /api/debate/reset, /api/debate/phase, /api/debate/end-arguments, GET /api/debate/ai-request, POST /api/debate/ai-result, and more")
        Component(quiz, "features/quiz", "FastAPI router", "POST/GET /api/quiz-request, POST /api/quiz-status, POST/DELETE /api/quiz-preview, POST/GET /api/quiz-refine")
        Component(summary, "features/summary", "FastAPI router", "POST/GET /api/summary, POST/GET /api/notes, POST /api/transcript-status, POST/GET /api/summary/force, POST /api/token-usage")
        Component(leaderboard, "features/leaderboard", "FastAPI router", "POST /api/leaderboard/show, POST /api/leaderboard/hide, DELETE /api/scores")
        Component(slides, "features/slides", "FastAPI router", "POST /api/slides/current, DELETE /api/slides/current, GET /api/slides, GET /api/slides/file/{slug}, POST /api/slides/upload")
        Component(session, "features/session", "FastAPI router", "POST /api/session/start|end|pause|resume|create, PATCH /api/session/rename, GET /api/session/request, POST /api/session/sync, GET /api/session/snapshot")
        Component(snapshot, "features/snapshot", "FastAPI router", "GET /api/state-snapshot, POST /api/state-restore")
        Component(pages, "features/pages", "FastAPI router", "GET / (participant.html), GET /host (host.html), GET /notes (notes.html)")
        Component(activity, "features/activity", "FastAPI router", "POST /api/activity (switches current_activity)")
    }
    ...
    Rel(emoji_overlay, ws, "WS /ws/__overlay__ (emoji reactions)", "WSS")
...
C4Component
    ...
    Container_Boundary(daemon_pkg, "Daemon (Python 3.12, host's Mac)") {
        Component(main, "daemon/__main__", "Orchestrator", "Starts all background loops. Graceful shutdown on SIGTERM. Exit code 42 triggers git pull + restart. On WS reconnect: re-syncs active session to backend.")
        Component(config_http, "daemon/config + daemon/http", "Config & HTTP", "Env vars: DEFAULT_TRANSCRIPT_MINUTES, SESSIONS_FOLDER, TRANSCRIPTION_FOLDER. Shared HTTP helper with Basic Auth headers.")
        Component(quiz_gen, "daemon/quiz/generator", "Quiz generator", "Reads transcript or topic, calls LLM, generates poll question + options.")
        Component(quiz_hist, "daemon/quiz/history", "Quiz history", "Tracks previously generated questions to avoid repetition.")
        Component(quiz_api, "daemon/quiz/poll_api", "Quiz poll API", "Posts quiz preview to backend. Polls /api/quiz-request and /api/quiz-refine.")
        Component(debate_ai, "daemon/debate/ai_cleanup", "Debate AI cleanup", "Deduplicates, fixes typos, suggests new arguments via LLM. Polls /api/debate/ai-request, posts result.")
        Component(summarizer, "daemon/summary/summarizer", "Summarizer", "Delta-based key-point extraction from transcript. Two-tier: notes + discussion points.")
        Component(summary_loop, "daemon/summary/loop", "Summary loop", "Polls /api/summary/force. Triggered by host or participant Key Points button.")
        Component(transcript_normalizer, "daemon/transcript/normalizer", "Transcript normalizer", "Incrementally normalizes raw transcript lines into daily YYYY-MM-DD transcription.txt files.")
        ...
        Component(transcript_timestamps, "daemon/transcript/timestamps", "Timestamp injector", "Auto-appends [HH:MM:SS] markers every ~3s to the active transcript file.")
        Component(transcript_loop, "daemon/transcript/loop", "Transcript loop", "Orchestrates normalization + timestamp injection.")
        Component(slides_catalog, "daemon/slides/catalog", "Slides catalog", "Reads materials_slides_catalog.json. Resolves PPTX to target PDF mappings.")
        ...
        Component(slides_loop, "daemon/slides/loop", "Slides loop", "Watches catalog for changes, triggers convert + upload pipeline.")
        Component(slides_daemon, "daemon/slides/daemon", "Slides daemon main", "Manages slide WS connection to backend. Handles upload requests and results.")
        Component(materials_mirror, "daemon/materials/mirror", "Materials mirror", "Mirrors project files to server_materials/. Keeps backend's RAG index fresh.")
        ...
        Component(rag_files, "daemon/rag/project_files", "Project files scanner", "Scans and lists project files. Handles Claude tool calls for file reading.")
        Component(llm, "daemon/llm/adapter", "LLM adapter", "Claude API wrapper. Token counting & cost tracking. create_message() with timeout.")
        Component(session_state, "daemon/session_state", "Session + global state", "Global state: training-assistant-global-state.json stores active_session_id. Session metadata in session_meta.json per folder. Persists full server snapshot to session_state.json.")
    ...
    Rel(quiz_api, fastapi, "GET /api/quiz-request, GET /api/quiz-refine", "HTTPS")
    ...
    Rel(quiz_api, fastapi, "POST /api/quiz-preview, POST /api/quiz-status", "HTTPS")
    Rel(debate_ai, fastapi, "GET /api/debate/ai-request, POST /api/debate/ai-result", "HTTPS")
    Rel(debate_ai, llm, "LLM call")
    Rel(summary_loop, fastapi, "GET /api/summary/force, POST /api/summary", "HTTPS")
    ...
    Rel(session_state, fastapi, "GET /api/session/snapshot, GET /api/session/request, POST /api/session/sync", "HTTPS")
    Rel(rag_indexer, rag_files, "uses")
    Rel(rag_retriever, rag_indexer, "queries")
    Rel(audio_hijack, transcript_normalizer, "Writes raw transcript (daemon reads local disk)", "Local file")
    Rel(llm, claude_api, "claude-haiku / claude-sonnet API calls", "HTTPS")
```
...
```mermaid
C4Component
    title Desktop Overlay and Wispr Addons — C3 Component Diagram
    ...
    Container_Boundary(overlay, "Emoji Overlay (Swift / AppKit, host's Mac)") {
        Component(app_delegate, "AppDelegate", "Swift / AppKit", "App entry point. Manages window lifecycle. Connects WebSocket to backend on launch.")
        Component(overlay_panel, "OverlayPanel", "NSPanel subclass", "Transparent always-on-top window. Covers full screen, ignores mouse events. PID lock file ensures single instance.")
        Component(emoji_animator, "EmojiAnimator", "Swift / AppKit", "Receives emoji reactions from WebSocket. Animates emoji sprites flying up the screen. Self-removing after animation completes.")
        Component(button_bar, "ButtonBar", "Swift / AppKit", "Small floating button bar. Host-triggered controls: sound effects, overlay show/hide toggle.")
        Component(sound_manager, "SoundManager", "Swift / AVFoundation", "Plays applause, drum roll, fanfare sounds. Triggered by host via ButtonBar.")
    }

    Container_Boundary(wispr, "Wispr Addons (Python, host's Mac)") {
        Component(clean_py, "wispr-addons/clean.py", "Python / pyobjc", "CGEventTap intercepts all keyboard and mouse events.")
        Component(clipboard_capture, "Clipboard capture", "Python / pyobjc", "Cmd+V: stores clipboard at each paste. Cmd+Ctrl+V: sends to Claude Haiku for cleanup, undoes original paste, re-pastes cleaned version. Cmd+Ctrl+Opt+V: same but adds contextual emojis.")
        Component(dictation_mute, "Dictation mute", "Python / pyobjc", "Mouse Button 5 (Wispr Flow dictation toggle): pauses media playback, lowers loopback volume. Escape while dictating: restores volume + media.")
    }
    ...
    Rel(app_delegate, fastapi, "WS connect as __overlay__, receives emoji_reaction events", "WSS")
    ...
    Rel(clipboard_capture, claude_api, "POST to Claude Haiku for grammar/filler cleanup", "HTTPS")
```
...
sequenceDiagram
    ...
    participant O as "Desktop Overlay"
    participant GD as "Google Drive"
    participant AI as "Claude API"

    Note over H,AI: 1. Session Lifecycle
    H->>B: POST /api/session/start {name}
    B-->>D: WS session_request {action:"start", name, request_id}
    D->>D: Create session folder on disk
    ...
    Note over H,AI: ... (participants join, session running) ...
    ...
    Note right of P: Paused participants reconnect automatically
    H->>B: POST /api/session/end
    B-->>D: WS session_request {action:"end", request_id}
    D->>D: Persist state to disk
    D-->>B: WS global_state_saved {request_id, persisted:true}
    Note over D,B: Backend proactively pushes state and session snapshots to daemon every 7s via WS for disk persistence.

    Note over H,AI: 2. Participant Join
    P->>B: WS connect /ws/{uuid}
    B->>P: WS accept
    P-->>B: WS set_name {name}
    B->>B: Store participant_names[uuid], assign_avatar(uuid)
    ...
    Note over H,AI: 3. Poll / Quiz Flow
    rect rgb(235, 245, 255)
        Note over H,P: Host-created poll
    ...
    rect rgb(235, 245, 255)
        Note over H,AI: Quiz generation (daemon)
    ...
    B->>B: Calculate speed-based scores (1000 max, 500 min, linear decay)
    B-->>P: WS result {correct_ids, voted_ids, score}
    B-->>H: WS broadcast (scores updated)

    Note over H,AI: 4. Q&A Flow
    P-->>B: WS qa_submit {text}
    B->>B: Store qa_questions[qid], award 100 pts to author
    B-->>P: WS broadcast (qa state)
    B-->>H: WS broadcast (qa state)
    P-->>B: WS qa_upvote {question_id}
    B->>B: Add to upvoters set, +50 pts to author, +25 pts to voter
    B-->>P: WS broadcast
    B-->>H: WS broadcast
    ...
    B->>B: Add to confirmed set, +200 pts per participant who flagged it
    B-->>P: WS broadcast (confirmed lines, scores)
    B-->>H: WS broadcast

    Note over H,AI: 7. Debate Flow
    H->>B: POST /api/debate {statement}
    B->>B: Reset debate state, phase = "side_selection"
    B-->>P: WS broadcast
    P-->>B: WS debate_pick_side {side: "for"|"against"}
    B->>B: Record side choice
    opt >= 50% picked
        B->>B: Auto-assign remaining to balance teams
    end
    opt All assigned
        B->>B: phase = "arguments"
    end
    ...
    B->>B: Store argument, +100 pts to author
    B-->>P: WS broadcast
    P-->>B: WS debate_upvote {argument_id}
    B->>B: +50 pts to author, +25 pts to voter
    B-->>P: WS broadcast
    ...
    B->>B: Apply merges, add AI args, phase = "prep"
    B-->>P: WS broadcast
    P-->>B: WS debate_volunteer
    B->>B: Set champion for side, +2500 pts
    B-->>P: WS broadcast
    H->>B: POST /api/debate/round-timer {index, seconds}
    B-->>P: WS debate_timer
    B-->>H: WS debate_timer

    Note over H,AI: 8. Slide Update Flow (PPTX change)
    D->>D: Watch PPTX folders for mtime changes
    ...
    B-->>H: WS broadcast
    Note over B: Slide catalog is pre-loaded at backend startup. Daemon sends slide_invalidated when PPTX files change.

    Note over H,AI: 9. Slide Loading Flow
    ...
    Note over H,AI: 15. Mode Switch
    H->>B: POST /api/mode {mode: "workshop"|"conference"}
    B->>B: Set mode, assign character names if switching to conference
    B-->>P: WS broadcast (mode changed)
    B-->>H: WS broadcast

    Note over H,AI: 16. Summary / Key Points Flow
    H->>B: POST /api/summary/force
    Note right of H: or participant triggers same endpoint (public, 30s cooldown)
    ...
    Note over H,AI: 19. Backend Restart Recovery
    Note over B: Backend restarts (deploy or crash) — in-memory state cleared
    ...
    D->>D: Load session_state.json from active session folder
    D-->>B: WS session_sync {main, key_points, session_state}
    B->>B: Restore session_main, scores, participants, activity state
