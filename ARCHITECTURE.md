...
7. Session materials zip (Google Drive firewall fallback)
   - Some clients block `drive.google.com`, which makes the participant sidebar's Google Drive entry useless for them. The download glyph next to it serves the same content through the Railway relay instead.
   - The participant calls `GET /{session_id}/api/materials/zip`. If [`railway/features/materials/router.py`](railway/features/materials/router.py) holds an archive younger than 60s it serves it straight from `.server-data/materials`.
   - Otherwise Railway pushes `build_materials_zip` over the daemon WebSocket and awaits a Future. Concurrent clicks share that one Future — the same dedup shape as `_pending_refresh` in the slides router — so a cohort of 25 costs one build, not 25.
   - [`daemon/materials/zip_builder.py`](daemon/materials/zip_builder.py) zips the local session folder (already on disk via the DriveFS mirror, so Google is never in the path), excluding `session-state.json`, `attendees.md`, and junk, and guarding at 25 MB. [`daemon/materials/upload.py`](daemon/materials/upload.py) POSTs it to `/api/materials/zip/upload` with host Basic auth — or posts an `error` field so waiting participants fail fast instead of sitting out the 20s timeout.
