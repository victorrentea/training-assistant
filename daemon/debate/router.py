...
async def pick_side(request: Request):
    ...
    if not debate_state.pick_side(pid, side):
...
async def submit_argument(request: Request):
...
async def upvote_argument(request: Request):
    ...
    body = await request.json()
    arg_id = body.get("argument_id")

    result = debate_state.upvote_argument(pid, arg_id)
    if result is None:
...
@host_router.post("")
async def launch_debate(request: Request):
...
async def reset_debate(request: Request):
...
async def close_selection(request: Request):
...
async def force_assign(request: Request):
...
async def advance_phase(request: Request):
    ...
    debate_state.advance_phase(phase)
    _broadcast({"type": "debate_updated", **debate_state.snapshot()})
    return JSONResponse({"ok": True, "phase": phase})


@host_router.post("/first-side")
async def set_first_side(request: Request):
    ...
    debate_state.set_first_side(side)
    _broadcast({"type": "debate_updated", **debate_state.snapshot()})
    return JSONResponse({"ok": True})


@host_router.post("/round-timer")
async def start_round_timer(request: Request):
    """Host starts a timed round."""
    body = await request.json()
    round_index = body.get("round_index")
    seconds = body.get("seconds")

    if round_index is None or seconds is None:
        return JSONResponse({"error": "round_index and seconds are required"}, status_code=400)

    debate_state.start_round(round_index, seconds)

    started_at = debate_state.round_timer_started_at.isoformat() if debate_state.round_timer_started_at else None
    _broadcast({"type": "debate_timer", "round_index": round_index, "seconds": seconds, "started_at": started_at})
    _broadcast({"type": "debate_updated", **debate_state.snapshot()})
    return JSONResponse({"ok": True})


@host_router.post("/end-round")
async def end_round(request: Request):
...
async def end_arguments(request: Request):
...
async def receive_ai_result(request: Request):
    """Manual/skip AI result — host posts AI cleanup results directly."""
    body = await request.json()
    merges = body.get("merges", [])
    cleaned = body.get("cleaned", [])
    new_arguments = body.get("new_arguments", [])

    debate_state.apply_ai_result(merges, cleaned, new_arguments)
    _broadcast({"type": "debate_updated", **debate_state.snapshot()})
    return JSONResponse({"ok": True})
