...
- [x] direct request (2026-06-08): emoji master switch — host footer ❤️ badge (green=on, gray=off) toggles a session-wide flag via `POST /api/{session_id}/host/emoji/global-toggle`; when off, the daemon silently drops all participant emoji reactions (host screen + macOS overlay) before any forwarding or counting, returning `204` so the participant stays unaware. Flag (`emoji_global_enabled`) lives on `participant_state` and persists in `session-state.json`. Per-participant blocking was deliberately dropped (a spammer can reuse another participant's UUID, so it's not bullet-proof). Also removed dead Railway `send_emoji_to_host`/`_send_to_special` — a latent unguarded emoji→host path.
- [ ] deferred security risk (accepted for now, surfaced while building the emoji master switch): a participant's UUID is their only credential (the `X-Participant-ID` header *is* the identity, with no validation it belongs to a registered participant), and UUIDs already leak to participant browsers via `ScoresUpdatedMsg`, Q&A (`author_uuid`/`upvoter_uuids`) and debate (`author_uuid`/`upvoters`) — enabling impersonation. Fix = anonymize participant-facing payloads (opaque per-session ids) + validate sender UUID against `participant_names`.
- [x] direct request (2026-06-08): per-participant emoji rate limit — the daemon `/api/participant/emoji/reaction` now throttles each participant to a burst of 15 reactions but no more than 15 per rolling minute (sliding-window limiter in `daemon/emoji/rate_limit.py`, keyed by participant id; `__`-prefixed ids such as `__host__` are exempt), returning `429` when exceeded. The participant page shows a debounced "Slow down 🐢" toast (at most once / 3s) on `429`; the optimistic local float still shows.
- [x] direct request (2026-06-08): emoji reaction whitelist as single source of truth — security hardening (pentest finding: any string ≤4 chars floated on host/overlay). New `daemon/emoji/catalog.py` holds the canonical `EMOJI_CATALOG` (emoji/title/section/badge) and derived `ALLOWED_EMOJI`; `/reaction` now rejects anything not in the catalog with `400`. The participant bar is rendered from the catalog (shipped on `GET /api/participant/state` as `emoji_catalog`) via `renderEmojiBar()` instead of hardcoded buttons, so the UI and the whitelist can never drift and the list lives only in Python.
- [x] bugfix (2026-06-02): the yellow access-duration toast ("This tool is available only during the training…") now appears at most once per day per device. It previously recorded its per-day localStorage flag (`new:access_toast_dismissed_date`) only when the user clicked the × to dismiss it, so re-joining (or a WS reconnect) the same day without dismissing re-showed it. `_showAccessToast()` now records the flag the moment the toast is shown. Added JS unit tests (behavioral once-per-day model + source-level guard).
- [x] feature (2026-06-01): participant engagement tracking — the participant page accumulates per-view active time / visits / clicks (active = tab visible + interaction within 60s) and flushes deltas to `POST /api/participant/activity` (≤ every 30s while active, plus on tab hide/unload), with the `X-Participant-ID` header. The daemon merges deltas into `PersistedParticipant.engagement` (persisted by the 3s snapshot loop) and stamps runtime `last_active_at` / `last_view`. Surfaced to the host on the existing `participant_list_updated` message (and `/host/state`); the host shows a `👁 N active` footer badge (live count derived locally from `last_active_at`, decaying on its own) with a cumulative per-view time/visits/clicks breakdown popover on hover.
- [x] direct request (2026-06-01): participant view now shows a non-blocking "break reminder" bubble above the ☕ button ("Remember to click this in case you need a break.") with a "Got it" link to dismiss (no auto-dismiss). It pops at most twice ever per device — 5 min and 60 min after the participant view opens — each guarded by its own localStorage flag (`workshop_break_reminder_5min_seen`, `workshop_break_reminder_60min_seen`). Tapping ☕ also dismisses it. Reuses the onboarding bubble styling and `_positionTooltipAboveCoffee()` (now parameterized by tooltip id).
- [x] direct request (2026-05-22): participant now auto-switches to Activity tab on every `quiz_opened` WS event (previously only from null/slides view), and the single-choice vote hint reads "Choose one option to vote." instead of "Choose an option to vote.".
- [x] bugfix (2026-04-19): landing page no longer infinite-loops when `ON_HOST_MACHINE` cookie is set and the local daemon reports a `session_id` the backend doesn't know (backend `state.session_id=None` → `/{code}/` bounced to `/?error=invalid` → repolled → redirected again). `pollLocalDaemonOnce` now validates the daemon's session against the backend via `/{code}/api/status` before navigating, and guards against re-entrant polls.
...
- [x] direct request: grouped persisted session activity fields into nested objects (`quiz`, `wordcloud`, `codereview`, `debate`) with backward-compatible read migration from legacy flat `*_` keys and nested restore compatibility.
- [x] direct request: `DB.md` generator no longer emits markdown tables; it now renders model sections as bullet lists (Global State first, Session State second) using the same shape-rendering helpers as API docs.
...
- [x] bugfix: persisted `session-state.json` now normalizes `quiz_correct_ids: null` to `[]` during Pydantic validation to avoid noisy validation errors and raw-fallback loads.
...
- [x] direct request: removed daemon polling calls to `/api/status` (startup + loop), including obsolete auto-update self-kill and needs_restore checks.
...
- [x] bugfix: host UI manual quiz `Send` (in `or enter manually`) works again; `/api/{session_id}/host/quiz` now accepts string options and normalizes them to `{id,text}`.
...
- [x] bugfix: host `Slides` mode (`activity=none`) now shows the center join panel again (`center-qr`) instead of incorrectly showing/hiding the quiz center panel.
...
- [x] Move the button to generate quiz next to the create quiz, right below it.
...
- [x] feat: display in the 'current quiz' area if the current quiz is multi or single select.
...
- [x] Feature: Allow the host to click items in the current quiz after closing the voting to mean correct options (one or multi) depending on what was chosen.
...
- [x] Feature from the host user interface to allow downloading all the quiz questions along with the correctly marked options by the host. All the quiz questions asked during the current day can be stored in local storage on the host browser.
...
- [x] Feat: Place the connected label into the div for "create quiz" To the right of the title  and remove the "host panel" title completely so that we gain more vertical space on the screen.
...
- [x] feature: Allow host to set a timeout for completing the quiz. This time out is pushed to the participants' view as a countdown timer, and the host can click any of four buttons setting a time out in 5 seconds, 10 seconds, 15, or 20 seconds. Clicking all four buttons makes them disappear, and a countdown down shows up in both the participants' screen and in the host view.
- [x] Feature: The proposed quiz coming from LLM from the daemon is currently correctly pushed without the correct options. The options suggested by the LLM are not displayed right now anywhere in the host UI. I want to change this. The moment I close the quiz, the voting as a host, I want to be suggested in a, let's say, subtle way which options LLM considers to be correct, for example by displaying a greyed out checkbox for example on the options in the current quiz.
...
- [x] bug: The hints about which is the correct option should only be displayed in the host view after the quiz is closed, and even then in a subtle suggestion, like a grade checkbox or a translucent suggestion somehow.
...
- [x] The three open four options to end the quiz should be displayed between the open voting and remove quiz, and have the same size and look and feel as the other buttons in yellow.
- [x] align the height of the connected divs, the reset scores, launch generate, open voting, remove quiz, quiz Q&A. All the buttons should have the same height.
- [x] bug: edit button in host Q&A doesnt work
- [x] bug: upon refresh of the host ui, the correct active activity is lost - It always goes back to quiz, leaving the center div displaying the correct activity, but the left div in host is always reset.
...
- [x] bug: Remove the download questions button that's from the bottom right corner in the host UI. Keep the buttons from the quiz content, which is already there. Shorten the text in that bottom right corner to one: say "join" and then the URL without HTTPS, and place the "reset scores" button to the right of the QR icon.
...
- [x] Show source reference (book/slides + page) in host UI for AI-generated quizzes — in both preview card and active quiz area.
...
- [x] ux: quiz host generate button now uses only two labels and toggles correctly: "Generate from transcript" and "Generate on topic".
...
- [x] allow participants to vote at quizzes using keyboard up+down in single select; enter submit vote. in multi-select, display a checkbox on each option line and allow selecting it using SPACE.
...
- [x] load test: 30 participants connected simultaneously, host fires a quiz, all vote randomly, host sorts by points. Run against local instance first, then deployed.
...
- [x] feat: browser push notifications — participant UI requests permission on Join; 🔔 button for auto-joiners; notifies on new quiz/Q&A/word cloud when tab is hidden
...
- [x] fix: participant vote and quiz results lost on browser refresh — server now sends `my_vote` and `quiz_correct_ids`/`my_voted_ids` in state broadcasts so vote + correct/incorrect feedback survive reconnect (GH #33)
...
- [x]  Și vreau să mai am un test de load sau de stres în plus, care conectează trei sute de participanți la instanța de producție.  Vreau să văd că pot să trimit un quiz pe care să voteze toți și să nu crapă sistemul.  Trebuie să fie un script derulat opțional, doar de pe local. Nu pe pipeline. Dar aș vrea să ții minte să-l rulezi ori de câte ori este posibil, înainte de push.
...
- Confidence Voting: After a quiz closes, ask each participant to rate their confidence (1–3 stars). Host sees a 2D scatter of opinion × confidence. Surfaces "strongly wrong" answers (high confidence, wrong option) — the richest material for debate.
...
- [x] Direct request: added 🍕 pizza reaction to the participant overflow emoji menu (right of 🤯); propagates through the existing generic emoji path to host + macOS addon overlay (no addon change needed — `spawnEmoji` renders any emoji).
- [x] Direct request: at session start, if no `.txt` notes file exists in the session folder, the daemon now auto-creates `<folder name> - notes.txt` (empty) and opens it in VS Code (`open -a "Visual Studio Code"` via macOS adapter); the file is then served as session notes. Existing notes files are used silently, never clobbered.
