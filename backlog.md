...
- [x] direct request (2026-05-22): participant now auto-switches to Activity tab on every `poll_opened` WS event (previously only from null/slides view), and the single-choice vote hint reads "Choose one option to vote." instead of "Choose an option to vote.".
...
- [x] bugfix: host UI manual poll `Send` (in `or enter manually`) works again; `/api/{session_id}/host/poll` now accepts string options and normalizes them to `{id,text}`.
