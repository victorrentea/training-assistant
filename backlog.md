...
- [x] Direct request: daemon now broadcasts live active PowerPoint presentation + slide (fullscreen-safe) to participants, and participant PDF.js auto-follows host deck/page.
...
- [x] Direct request: aligned local slides defaults with daemon publish flow (`.context/published-slides` / `PPTX_PUBLISH_DIR`) so local testing no longer depends on source `materials/slides`.
...
- [x] Direct request: added Slides download progress bar in participant modal and HTTP caching headers (`ETag`, `Last-Modified`, `Cache-Control`) with `304` support for `/api/slides/file/{slug}`.
...
- [x] Direct request: participant top badge widened (~+60%), waiting subtitle no longer clipped, and slides list moved to fixed right-side overlay (without pushing layout columns); proof: `.context/screenshots/participant-overlay-wide-8011.png` and stress check `.context/screenshots/participant-overlay-wide-stress-2-8011.png`.
- [x] Follow-up direct request: kept right slides list visible on narrower widths (removed mobile hide), and constrained slides PDF overlay to stop before right dock (`right: var(--slides-overlay-width)`), so titles remain visible.
...
- [x] Direct request: tightened spacing between slide title and download button, added uniform topic distribution when list has up to 10 entries, reserved 20px bottom clearance to avoid status overlap, and forced visible slide-list scrollbar on short screens (<720px height).
- [x] Direct request: enlarged slide page-number typography and top-right close `X` with matching font style, removed boxed-button look for `X`, moved page indicator to bottom-left, and removed "with Santa" suffix from version/work-hours status text.
